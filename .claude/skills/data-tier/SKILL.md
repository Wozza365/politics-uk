...
4. **Wire it up**: `src/stores/scenario.ts` getters, a `ViewSwitcher.vue` nav entry, the view
   component — rendered only through `src/map/MapRenderer.ts`, never direct SVG/DOM access.

Also required, every time:
- Add a `validate-scenario.mjs` check that the new tier's seat/membership count reconciles to the
  known total for that body.
