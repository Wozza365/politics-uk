# Politics UK — Game Specification (Draft v0.1)
...
| **Tier** | A level of UK governance (Commons, Lords, Holyrood, councils, parishes…). See [§4](#4-the-uk-political-data-model). |
...
Priority reflects how readily real data exists and how central it is to MVP, **not** importance.

| # | Tier | Units (≈ count, Jan 2025) | Seats | MVP? | Data difficulty |
| --- | --- | --- | --- | --- | --- |
| 1 | **House of Commons** | 650 constituencies (2024 boundaries) | 650 MPs | ✅ MVP | Easy |
| 2 | **House of Lords** | n/a (chamber) | ~800 peers by party group | ✅ MVP (stats only) | Easy |
| 3 | **Scottish Parliament** | 73 constituencies + 8 regions (56 list) | 129 MSPs | ✅ MVP | Easy |
| 4 | **Senedd (Wales)** | 40 constituencies + 5 regions (20 list)¹ | 60 MS | ✅ MVP | Easy |
| 5 | **NI Assembly** | 18 constituencies (5 each) | 90 MLAs | ✅ MVP | Easy |
| 6 | **London Assembly** | 14 constituencies + London-wide list | 25 AMs | ◻ Later | Easy |
| 7 | **Combined-authority / metro mayors** | ~12 mayoralties | 1 each | ◻ Later | Easy |
| 8 | **Directly elected local mayors** | ~15 | 1 each | ◻ Later | Medium |
| 9 | **Police & Crime Commissioners** | ~37 (Eng & Wales) | 1 each | ◻ Later | Easy |
| 10 | **County councils** | ~21 | wards | ◻ Later | Medium |
| 11 | **District / borough councils** | ~164 | wards | ◻ Later | Medium |
| 12 | **Unitary authorities** | ~62 | wards | ◻ Later | Medium |
| 13 | **Metropolitan boroughs** | 36 | wards | ◻ Later | Medium |
| 14 | **London boroughs** | 32 (+ City of London) | wards | ◻ Later | Medium |
| 15 | **Scottish councils** | 32 | wards | ◻ Later | Medium |
| 16 | **Welsh councils** | 22 | wards | ◻ Later | Medium |
| 17 | **NI councils** | 11 | wards | ◻ Later | Medium |
| 18 | **Parish / town / community councils** | ~10,000+ | seats | ◻ Later (best-effort) | **Hard** (sparse, non-uniform data) |
...
> **Reality check on "EVERYTHING":** tiers 1–9 are very achievable from open data. Council
> _composition_ (tiers 10–17) is well covered by Open Council Data / Wikipedia. **Parish councils
> (tier 18)** are the long tail — ~10k bodies, many uncontested, no single clean dataset. The plan
> treats them as a best-effort, post-MVP aggregate rather than a blocker. See [§5](#5-data-acquisition-plan).
...
> **Provenance matters.** Where a value is estimated (finance, membership, parish aggregates), the
...
### 7.2 Party selector
- A row/grid of **party cards**, one per party **with representation**, **excluding local/independent
  parties** (regional parties like **SNP** and **Plaid Cymru** _are_ included).
...
### 9.3 Top-centre — party at-a-glance _(MVP)_
- **Party name.**
- **Current polling %** (rounded to 0 dp).
- **Seats** — two figures: **Parliament (Commons)** prominent, and a **combined "everything else"**
  total slightly smaller.
- **Party finance** (estimated; flagged).
- **Party membership.**
- **Suggested additions** (please confirm — [§13](#13-open-questions)): leader approval rating,
  vote-share trend arrow (momentum), number of councils controlled, days since last election.
...
- Visible **clock** showing the simulated date; **ticks daily**.
...
- **Clock tick (daily):** advances date; rolls for events from a weighted pool.
- **Event sources:**
...
### 11.2 Win conditions differ by party scope
- **National parties** (Lab, Con, LD, Reform, Green E&W…): win/lead a **UK General Election** /
  form a government.
- **Regional parties** (SNP, Plaid Cymru, NI parties): they don't contest all 650 seats, so "win the
  UK GE" is the wrong goal. Proposed: their win condition is **dominance of their devolved nation**
  (largest party / first minister), with optional stretch goals (e.g. independence referendum).
  **Needs your confirmation** — [§13](#13-open-questions).
...
**Highest impact**
1. **Win conditions for regional parties** (SNP/PC/NI) — confirm "dominate devolved nation" vs other.
2. **What does a turn feel like?** Pure daily auto-advance until an action is needed, or can the
   player take proactive actions (campaign, spend, policy) between events? This defines the core loop.
3. **Player levers** — what can the player actually _do_ (spend money, set policy, campaign in
   regions, manage the party)? MVP can be event-response-only, but I need to know the direction.

**Medium impact**
4. **Top-centre stats** — approve the suggested extras (leader approval, momentum arrow, councils
   controlled, days-to-election)?
5. **Feed ordering** — newest-first (assumed) or chronological?
6. **Difficulty scale** — 3 bands (Easy/Medium/Hard) or 5? And do you want me to hand-tune Jan-2025
   ratings or derive purely by formula?
7. **Polling model** — is the headline % a static scenario value for now, or should it drift with
   events from day one?

**Lower impact / later**
8. **Event format** — happy for me to design the event schema, or do you have a structure in mind?
9. **Party finance** — acceptable to show estimates with a footnote where real figures are absent?
10. **Parish councils** — confirm they're best-effort/aggregate, not a Phase-1 requirement.
11. **Working title** — "Politics UK" is a placeholder; any preferred name?
