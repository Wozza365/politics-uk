...
function pickKnownLeverCooldowns(record: Record<string, ISODate>, knownPartyIds: Set<PartyId>): Record<string, ISODate> {
  ...
  for (const [key, date] of Object.entries(record)) {
    ...
    if (knownPartyIds.has(partyId) && leverId in LEVER_ACTIONS) result[key] = date
  }
  return result
}

/** Drops commitments referencing a party the current scenario no longer recognises (P3.0's
 * hydration pattern — see `pickKnownParties`) — every commitment today comes from a lever, so
 * `actionId` is checked against `LEVER_ACTIONS` the same way.  */
function pickKnownCommitments(commitments: ActiveCommitment[], knownPartyIds: Set<PartyId>): ActiveCommitment[] {
  return commitments.filter((commitment) => knownPartyIds.has(commitment.partyId) && commitment.actionId in LEVER_ACTIONS)
}
...
export const useGameStore = defineStore('game', {
  ...
  actions: {
    ...
    cancelCommitment(commitmentId: string) {
      ...
      const label = LEVER_ACTIONS[commitment.actionId as LeverId]?.label ?? commitment.actionId
      this.recordFeedEntry({
